document.addEventListener("DOMContentLoaded", () => {
  ...
  const btnVerInfo = document.getElementById("btnVerInfo") as HTMLButtonElement;
  const btnVerTareas = document.getElementById("btnVerTareas") as HTMLButtonElement;
  ...
  function guardarTareas(tareas: any[]) {
  ...
  btnVerTareas.addEventListener("click", () => {
    ...
    const tareasEmpleado = tareas.filter(
      (t: any) => t.asignadaA.toLowerCase() === empleado.nombre.toLowerCase()
    );
    ...
    if (tareasEmpleado.length === 0) {
      ...
      tareasEmpleado.forEach((t: any, index: number) => {
        html += `
          <li>
      ...
      document.querySelectorAll(".btnCompletar").forEach(btn => {
        btn.addEventListener("click", (e) => {
          ...
          if (idx !== null) {
            const tareaGlobalIndex = tareas.findIndex(
              (tg: any) =>
                tg.asignadaA.toLowerCase() === empleado.nombre.toLowerCase() &&
